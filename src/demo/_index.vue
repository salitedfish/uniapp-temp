<template>
  <view class="content">
    ...
    <view v-html="originTemplate" class="template"></view>
...
<script>
import { getPrintParams, printPage } from "../../store/print"
...
// 实际数据
const printData = {
  shortUrl: "http://tmlmcloud.com:8084/url/J9cV&tenantCode=yxvbjkf",
  belongOrgName: "所属组织",
  cardCode: "卡片编号",
  cardName: "卡片名称",
...
export default {
  ...
  methods: {
    setPrint(num) {
      if (num == 0) {
        this.originContentString = originContentStringZero
      }
      if (num == 1) {
        this.originContentString = originContentStringOne
      }
      if (num == 2) {
        this.originContentString = originContentStringTwo
      }
      if (num == 3) {
        this.originContentString = originContentStringThree
      }
      if (num == 4) {
        this.originContentString = originContentStringFour
      }
      this.originContent = JSON.parse(this.originContentString)
      this.originTemplate = this.originContent.template

      for (const label in labelToKey) {
        const key = labelToKey[label]
        this.originTemplate = this.originTemplate.replace(
          label,
          this.printData[key]
        )
      }
    ...
    startPrint(tsplTemplate) {
      ...
      const { deviceId, serviceId, characteristic, characteristicId } =
        getPrintParams()
      printer.setPrinterParams({
        deviceId,
        serviceId,
        characteristic,
        characteristicId,
      ...
      console.log("最终tspl：", tsplTemplateWithData)
...
import { mmToDot } from "./template/TSPL"
