	...
	const scanSuccess = useThrottle(async (code: string) => {
		if (code) {
			...
			try {
				...
				if (res && res.data) {
					if (res.data.cWhCode !== config.value.inStockroomSelected[0].code) {
						resetFormShelf()
						uni.showToast({
							title: "所选转入仓库不存在此货位",
							icon: "none"
		...
		try {
			...
			const params = {
				body: tableData.value.map(item => {
					return {
						batch: item.batch,
						inPosCode: item.position,
						invCode: item.invCode,
						outPosCode: item.oriPosition,
						quantity: item.count
					}
				}),
				head: {
...
<template>
	<view class="common-page-container">
		...
		<view class="btn-box">
			<up-button type="primary" text="提交调拨" class="bottom-button" shape="circle" @click="submit"
