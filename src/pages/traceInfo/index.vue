<template>
  <view class="common-page-container">
    ...
    <up-steps :current="form.activeNum" style="margin-top: 10px; overflow-x: scroll">
      <up-steps-item v-for="(item, key) in processList" :key="key" :title="item.procedureName" :error="item.result != 1 && key != form.activeNum"></up-steps-item>
    </up-steps>

    <!-- 基础数据表单 -->
    <up-form class="common-form" :class="{ 'common-form-next': form.procedureCode === P.打包 && Platform.isApp() }" labelPosition="left">
      ...
      <up-form-item class="common-form-item" label="计划单号:" borderBottom labelWidth="80" style="padding: 0" required>
        <up-input-scan v-model="form.planCode" placeholder="请输入计划单号" clearable class="input-item" @scanSuccess="getPlanInfo"></up-input-scan>
      </up-form-item>

      <up-form-item class="common-form-item" label="产品编码:" borderBottom labelWidth="80" style="padding: 0">
        <up-input v-model="form.productCode" placeholder="计划单号自动带出" clearable class="input-item" readonly border="none"></up-input>
      </up-form-item>

      <up-form-item class="common-form-item" label="客户件号:" borderBottom labelWidth="80" style="padding: 0">
        <up-input v-model="form.cusProductCode" placeholder="计划单号自动带出" clearable class="input-item" readonly border="none"></up-input>
      </up-form-item>

      <up-form-item class="common-form-item" label="计划数量:" borderBottom labelWidth="80" style="padding: 0">
        <up-input v-model="form.planNum" placeholder="计划单号自动带出" clearable class="input-item" readonly border="none"></up-input>
      </up-form-item>

      <up-form-item class="common-form-item" label="实际数量:" borderBottom labelWidth="80" style="padding: 0">
        <up-input v-model="form.realNum" placeholder="计划单号自动带出" clearable class="input-item" readonly border="none"></up-input>
      </up-form-item>

      <up-form-item class="common-form-item" label="装箱数量:" borderBottom labelWidth="80" style="padding: 0" v-if="[P.打包].includes(form.procedureCode)">
        <up-input v-model="form.packageNum" placeholder="自动带出" clearable class="input-item" readonly border="none"></up-input>
      </up-form-item>

      <!-- ----------------------------------------根据不同模版显示不同选项------------------------------------- -->
      <up-form-item class="common-form-item" label="检具编码:" borderBottom labelWidth="80" style="padding: 0" required v-if="form.procedureCode == P.定制流程1">
        <up-input-scan v-model="form.barcode2" placeholder="请输入条码" clearable class="input-item" @scanSuccess="barcode2ScanSuccess"></up-input-scan>
      </up-form-item>

      <up-form-item class="common-form-item" label="包装码:" borderBottom labelWidth="80" style="padding: 0" required v-if="form.procedureCode == P.定制流程2">
        <up-input-scan v-model="form.barcode2" placeholder="请输入条码" clearable class="input-item" @scanSuccess="barcode2ScanSuccess"></up-input-scan>
      </up-form-item>

      <up-form-item class="common-form-item" label="标签码:" borderBottom labelWidth="80" style="padding: 0" required v-if="form.procedureCode == P.定制流程3">
        <up-input-scan v-model="form.barcode2" placeholder="请输入条码" clearable class="input-item" @scanSuccess="barcode2ScanSuccess"></up-input-scan>
      </up-form-item>
      ...
      >
        <up-input-scan
          ...
          @blur="item.inputFocus = false"
        ></up-input-scan>
      </up-form-item>

      <up-form-item
        class="common-form-item"
        :label="`${form.barcodeName1}:`"
        borderBottom
        labelWidth="80"
        style="padding: 0"
        :required="form.scanCode !== 1"
        v-if="![P.条码自配, P.打包, P.终检].includes(form.procedureCode)"
      >
        <!-- 后端配了必须扫码才必须要扫码 -->
        <up-input-scan v-model="form.barcode1" placeholder="请输入条码" clearable class="input-item" @scanSuccess="barcode1ScanSuccess"></up-input-scan>
      </up-form-item>

      <up-form-item class="common-form-item" :label="`${form.barcodeName2}:`" borderBottom labelWidth="80" style="padding: 0" required v-if="form.procedureKindCode == PK.条码关联">
        <up-input-scan v-model="form.barcode2" placeholder="请输入条码" clearable class="input-item" @scanSuccess="barcode2ScanSuccess"></up-input-scan>
      </up-form-item>
      ...
      <up-form-item class="common-form-item" label="客供条码:" borderBottom labelWidth="80" style="padding: 0" v-if="form.procedureCode == P.条码关联返工">
        <up-input v-model="form.barcode2" placeholder="请输入条码" clearable class="input-item"></up-input>
      </up-form-item>
    ...
    <CheckRecord :checkItems="checkRecords" :form="form" @setFormResult="setFormResult" @setFailureMod="setFailureMod" v-if="checkRecords && checkRecords.length"> </CheckRecord>
    ...
    <up-form class="common-form common-form-next" labelPosition="left">
      ...
      <up-form-item class="common-form-item" label="是否合格:" borderBottom labelWidth="80" style="padding: 0" required v-if="form.statistics == '1'">
      ...
      <up-form-item class="common-form-item" label="失效模式:" borderBottom labelWidth="80" style="padding: 0" required v-if="form.result === '0'">
        <TraceFailureModPicker
          v-model:selected="failureModSelected"
          @select="failuerModSelect"
      ...
      >
        <up-input v-model="form.equipmentCode" @change="equipmentCodeChange" placeholder="请选择设备" class="input-item" clearable>
          <template #suffix>
            <uni-icons custom-prefix="custom-icon" type="icon-chaxun" size="18" :color="globalColor.primary" @click="openEquipmentPicker"></uni-icons>
            <up-picker :show="showEquipmentPicker" :columns="equipments" @confirm="equipmentSelect" @cancel="showEquipmentPicker = false" keyName="equipmentName"></up-picker>
          </template>
        </up-input>
      </up-form-item>
      <up-form-item class="common-form-item" label="设备名称:" borderBottom labelWidth="80" style="padding: 0" v-if="[P.条码自配].includes(form.procedureCode) && equipments && equipments.length > 0">
        <up-input v-model="form.equipmentName" placeholder="自动带出" readonly class="input-item" border="none"></up-input>
      </up-form-item>
      <up-form-item class="common-form-item" label="设备数据:" borderBottom labelWidth="80" style="padding: 0" v-if="[P.条码自配].includes(form.procedureCode) && equipments && equipments.length > 0">
        <up-textarea v-model="form.equipmentContent" placeholder="自动带出" class="input-item" disabled></up-textarea>
      </up-form-item>
      <up-form-item class="common-form-item" label="结果:" borderBottom labelWidth="80" style="padding: 0" v-if="[P.条码自配].includes(form.procedureCode) && equipments && equipments.length > 0">
        <up-input :modelValue="ResultMap[form.result]" placeholder="自动带出" readonly class="input-item" border="none"></up-input>
      </up-form-item>

      <!-- 终检这步需要显示另一台设备的数据------------------------------------------------------------------------------------------- -->
      <up-form-item class="common-form-item" label="设备编码:" borderBottom labelWidth="80" style="padding: 0" v-if="[P.终检].includes(form.procedureCode)">
        <up-input v-model="form.equipmentCode1" placeholder="自动带出" readonly class="input-item" border="none"></up-input>
      </up-form-item>
      <up-form-item class="common-form-item" label="设备名称:" borderBottom labelWidth="80" style="padding: 0" v-if="[P.终检].includes(form.procedureCode)">
        <up-input v-model="form.equipmentName1" placeholder="自动带出" readonly class="input-item" border="none"></up-input>
      </up-form-item>
      <up-form-item class="common-form-item" label="设备数据:" borderBottom labelWidth="80" style="padding: 0" v-if="[P.终检].includes(form.procedureCode)">
        <up-textarea v-model="form.equipmentContent1" placeholder="自动带出" class="input-item" disabled></up-textarea>
      </up-form-item>
      <up-form-item class="common-form-item" label="结果:" borderBottom labelWidth="80" style="padding: 0" v-if="[P.终检].includes(form.procedureCode)">
        <up-input :modelValue="ResultMap[form.result1]" placeholder="自动带出" readonly class="input-item" border="none"></up-input>
      </up-form-item>
      ...
      <up-form-item class="common-form-item" label="结果:" borderBottom labelWidth="80" style="padding: 0" v-if="[P.终检].includes(form.procedureCode)">
        <up-input :modelValue="ResultMap[form.result2]" placeholder="自动带出" readonly class="input-item" border="none"></up-input>
      </up-form-item>
      <!-- ------------------------------------------------------------------------------------------------------------------ -->

      <up-form-item class="common-form-item" label="工装/模具:" borderBottom labelWidth="80" style="padding: 0" v-if="![P.条码自配, P.打包, P.终检].includes(form.procedureCode)">
        <up-input-scan v-model="form.toolingCode" @scanSuccess="toolingScanSuccess" placeholder="请输入工装/模具" clearable class="input-item"></up-input-scan>
      </up-form-item>
    </up-form>
    <!-- ----------------------------------------end---------------------------------------- -->
    ...
    <view v-if="form.procedureKindCode === PK.数据采集 && equipmentLoading" style="text-align: center; margin-top: 5px" class="success">设备数据采集中...</view>
    ...
    <view class="btn-box" v-if="form.procedureKindCode === PK.定制流程">
      <up-button type="primary" class="btn" shape="circle" v-if="form.procedureCode === P.定制流程1" @click="submit">检具检测</up-button>
      <up-button type="primary" class="btn" shape="circle" v-else-if="form.procedureCode === P.定制流程2" @click="submit">包装检测</up-button>
      <up-button type="primary" class="btn" shape="circle" v-else-if="form.procedureCode === P.定制流程3" @click="submit">标签码检测</up-button>
    ...
    <view class="btn-box" v-if="form.procedureCode === P.打包">
      <BoxBarcodePrinter :productCode="form.productCode" :cusProductCode="form.cusProductCode" :codeRule="codeRules[1]" v-if="codeRules[1]" @genSuccess="boxBarcodeGenSuccess"> </BoxBarcodePrinter>
    ...
    <view class="btn-box" v-if="form.printed == 1">
      <BoxBarcodePrinter1 :productCode="form.productCode" :cusProductCode="form.cusProductCode" :codeRule="codeRules[1]" v-if="codeRules[1]" @genSuccess="boxBarcodeGenSuccess"> </BoxBarcodePrinter1>
      <BoxBarcodeRePrinter1 style="margin-left: 5px"> </BoxBarcodeRePrinter1>
    </view>

    <!-- 表格 -->
    <TraceTablePK1 ref="traceTableRef" :form="form" style="margin-top: 10px" v-if="form.procedureKindCode === PK.条码关联"> </TraceTablePK1>
    <TraceTablePK2 ref="traceTableRef" :form="form" style="margin-top: 10px" v-if="form.procedureKindCode === PK.数据采集"> </TraceTablePK2>
    <TraceTablePK3 ref="traceTableRef" :form="form" style="margin-top: 10px" v-if="form.procedureKindCode === PK.扫码登记"> </TraceTablePK3>
    <TraceTablePK4 ref="traceTableRef" :form="form" style="margin-top: 10px" v-if="form.procedureKindCode === PK.定制流程 && ![P.条码自配, P.打包, P.终检].includes(form.procedureCode)">
    </TraceTablePK4>
    <TraceTablePK5 ref="traceTableRef" :form="form" :codeRules="codeRules" style="margin-top: 10px" v-if="[P.条码自配, P.打包, P.终检].includes(form.procedureCode)"> </TraceTablePK5>
...
import { printBoxBarcode } from "./util"
// @ts-ignore
...
import { getBarcodeList } from "@/api/codeRule"
// 静态数据
