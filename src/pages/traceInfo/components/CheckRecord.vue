...
<template>
  <view class="common-form common-form-next" labelPosition="left" :labelStyle="{ 'font-size': '14px' }" v-for="(item, key) in checkItems" :key="key">
    <up-cell-group>
      ...
      <up-cell v-if="item.filePictureUrl">
        ...
        <template #value>
          <view class="u-cell-text cell-value">
            <up-image
              v-for="(imgSrc, key) in item.filePictureUrl.split(',')"
              :key="key"
              :show-loading="true"
              :src="imgSrc"
              width="80px"
              ...
              @click="imgPreview(item.filePictureUrl)"
            ></up-image>
          </view>
        </template>
      </up-cell>
      ...
      <up-cell v-if="item.needSurvey == '1'">
        ...
        <template #value>
          <view class="u-cell-text cell-value">
            <text style="margin-right: 5px">({{ Number(item.minSurvey) }} - {{ Number(item.maxSurvey) }})</text>
            <up-input placeholder="请输入测量值" border="surround" v-model="item.surveyValue" type="number" @change="surveyValueChange(item)"></up-input>
          </view>
        </template>
      </up-cell>
      ...
      <up-cell v-if="item.result == '0'">
        ...
        <template #value>
          <TraceFailureModPicker
            v-model:selected="item.failureModSelected"
            @select="(mods: Objs) => failuerModSelect(item, mods)"
      ...
      <up-cell v-if="item.result == '0'">
        ...
        <template #value>
          <view class="u-cell-text cell-value">
            <up-textarea placeholder="请输入不合格理由" border="surround" style="text-align: left" v-model="item.reason"></up-textarea>
          </view>
        </template>
      </up-cell>
    </up-cell-group>
