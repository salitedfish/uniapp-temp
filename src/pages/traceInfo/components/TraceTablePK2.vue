<script lang='ts' setup>
	...
	const {
		...
		reSetPage,
		reSetList
	} = useTable(getAirtightnessTracePageList)
	...
	<view class="common-table">
		<uni-table border stripe emptyText="暂无更多数据" :loading="searching">
			...
			<uni-tr>
				...
				<uni-th class="nowrap" align="left" width="100rpx">测试通道</uni-th>
				<uni-th class="nowrap" align="left" width="100rpx">泄露值</uni-th>
			...
			<uni-tr v-for="item,key in resultData?.list" :key="key">
				...
				<uni-td class="nowrap">{{ item.barcode }}</uni-td>
				<uni-td class="nowrap">{{ item.etPass }}</uni-td>
				<uni-td class="nowrap">{{ item.status }}</uni-td>
