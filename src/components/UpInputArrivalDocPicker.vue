<script lang="ts" setup>
	...
	import { getPurchaseArrivalList } from "@/api/purchaseArrival"
	import type { ArrivalDoc, Business, Materiel } from "@/type/business"
	...
	const emit = defineEmits<{
		(event : "select", result : ArrivalDoc[]) : void;
		(event : "update:selected", result : ArrivalDoc[]) : void;
	}>()

	onMounted(() => {
		if (props.selected) {
			emit("select", props.selected as ArrivalDoc[])
	...
	// 生成分页所需的数据和方法
	const { searching, searchParam, resultData, searchList, reSetPage, reSetList } = useTable(getPurchaseArrivalList)
	...
	const select = (selected : ArrivalDoc[]) => {
		// emit("select", selected)
		close()
	}
	const updateSelected = (selected : ArrivalDoc[]) => {
		emit("update:selected", selected)
	}

	const colums = [{ label: "单号", key: "code" }, { label: "单据日期", key: "name" }, { label: "供应商", key: "name" }, { label: "部门", key: "name" }, { label: "制单人", key: "name" }]
