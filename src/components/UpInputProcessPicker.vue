<script lang="ts" setup>
	...
	import type { Process } from "@/type/business"
	...
	const emit = defineEmits<{
		(event : "select", result : Process[]) : void;
		(event : "update:selected", selected : Process[]) : void
	}>()

	onMounted(() => {
		if (props.selected) {
			emit("select", props.selected as Process[])
	...
	// 生成分页所需的数据和方法
	const { searching, searchParam, resultData, searchList, reSetPage } = useTable(getLdProcessList)
	...
	const select = (selected : Process[]) => {
		// emit("select", selected)
		close()
	}
	const updateSelected = (selected : Process[]) => {
